...
class ScoringStrategyTest {
    ...
    ScoringStrategy strategy = new ScoringStrategy(1000);

    @Test
    void getScoreFromValues() {

        assertThat(strategy.getScoreFromValues(new int[]{2, 2, 2}), equalTo(1.0));
        assertThat(strategy.getScoreFromValues(new int[]{1, 1, 2}), equalTo((double)2 / (double)3));
        assertThat(strategy.getScoreFromValues(new int[]{1, 2, 3}), equalTo((double)1/ (double)3));
    }

    @Test
    void getRubikSideScore() throws Exception {
        RubikSide side = new RubikSide(3, 1);
        assertThat(strategy.getRubikSideScore(side), equalTo(1.0));
        ...
        double expected = BigDecimal.valueOf(Math.max(expectedRow, expectedCol))
                                    ...
                                    .doubleValue();
        assertThat(strategy.getRubikSideScore(side), equalTo(expected));
    ...
    void getRubikCubeScore() {
        ...
        assertThat(strategy.getRubikCubeScore(cube), equalTo(1.0));
        ...
        double expected = BigDecimal.valueOf((1.0 + 1.0 + (2.0 / 3.0) + (2.0 / 3.0) + (2.0 / 3.0) + (2.0 / 3.0)) / 6.0)
                .setScale(3, RoundingMode.HALF_UP)
                .doubleValue();
        assertThat(strategy.getRubikCubeScore(cube), equalTo(expected));
    }

    @Test
    void getRubikSideCompletionScore() throws Exception {
        RubikSide side = new RubikSide(3, 1);
        assertThat(strategy.getRubikSideCompletionScore(side, side::getRow), equalTo(1.0));

        side.setRow(0, new int[]{1, 2, 3});
        side.setRow(1, new int[]{1, 1, 1});
        side.setRow(2, new int[]{1, 1, 1});
        double expected = 2.0 / 3.0;
        assertThat(strategy.getRubikSideCompletionScore(side, side::getRow), equalTo(expected));
        ...
        expected = 2.0 / 3.0;
        assertThat(strategy.getRubikSideCompletionScore(side, side::getCol), equalTo(expected));
    ...
    void getRubikSideAverageScore() throws Exception {
        RubikSide side = new RubikSide(3, 1);
        double expected = 1.0;
        assertThat(strategy.getRubikSideAverageScore(side, side::getRow), equalTo(expected));
        ...
        side = new RubikSide(3, 1);
        expected = 1.0;
        assertThat(strategy.getRubikSideAverageScore(side, side::getCol), equalTo(expected));
